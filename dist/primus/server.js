...
exports.primus = (() => {
    ...
    primus.joinGuildChat = (player) => {
        ...
        _.each(primus.players[player.name], spark => {
            if (!spark || spark.readyState === 2)
                return;
            try {
                spark.join(`chat:channel:Guild:${player.guildName}`);
            }
            catch (e) {
                logger_1.Logger.error('Primus:JoinGuildChat', e);
            }
        });
    };
    primus.leaveGuildChat = (player) => {
        ...
        _.each(primus.players[player.name], spark => {
            ...
            try {
                ...
                logger_1.Logger.error('Primus:LeaveGuildChat', e);
            }
        });
    };
    ...
    primus.on('connection', spark => {
        ...
        spark.on('roomserror', e => {
            logger_1.Logger.error('Spark:Rooms', e);
        });
        setTimeout(() => {
    ...
    primus.on('roomserror', (error) => {
        logger_1.Logger.error('Primus:Rooms', error);
    });
